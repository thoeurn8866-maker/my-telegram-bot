...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"ជម្រាបសួរ {user_first_name}! ខ្ញុំជា Bot បង្កើតឡើងដោយការិយាល័យបុគ្គលិកនៃនាយកដ្ឋានរដ្ឋបាល និងធនធានមនុស្ស ដើម្បីផ្តល់បច្ចុប្បន្នភាពបញ្ជីឈ្មោះនៃចំនួន 
        សហព័ន្ធ(ប្តី-ប្រពន្ធ ពុំមានមុខរបរ) 
        និងកូនក្នុងបន្ទុក ដែលមានអាយុពី ១៥ឆ្នាំ ដល់២៥ឆ្នាំ(ជាសិស្ស-និស្សិត)  របស់ថ្នាក់ដឺកនាំ-និយោជិត ទ.ក. ទាំងអស់ គិតមកដល់ថ្ងៃ២៤-០៧-២០២៦ 
        សម្រាប់យកមកធ្វើការគណនាបន្ថយមូលដ្ឋានកំណត់ពន្ធ ចំណាយលើប្រាក់ពន្ធបៀវត្សត្រូវបង់ប្រចាំខែឆ្នាំរបស់ថ្នាក់ដឺកនាំ-និយោជិត ទ.ក. ទាំងអស់ 
        ជូនអគ្គនាយកដ្ឋានពន្ធដារ និងរក្សាទុកយោងសម្រាប់ការបំភ្លឺផ្តល់ជូនភាគីពាក់ព័ន្ធ នៅពេលរងការធ្វើសវនកម្ម​ក្នុងការិយបរិច្ឆេទឆ្នាំនីមួយៗ។\n\n"
        f"សូមបញ្ចូល **ឈ្មោះប្តី ឬប្រពន្ធ មុខរបរនិងអាយុ** របស់អ្នក៖"
    )
    return SPOUSE_NAME
...
async def get_spouse_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("សូមបញ្ចូល **ឈ្មោះកូន ១៥ឆ្នាំ ដល់២៥ឆ្នាំ(ជាសិស្ស-និស្សិត) និងអាយុ ** របស់អ្នក (បើមានច្រើន បំបែកដោយសញ្ញាក្បៀស `,`)៖")
...
async def get_children_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("សូមបញ្ចូល **លេខទូរសព្ទ**៖")
    return phone_line
...
async def save_to_excel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    new_data = {
        ...
        'លេខទូរសព្ទ': [context.user_data['phone_line']]
    }
